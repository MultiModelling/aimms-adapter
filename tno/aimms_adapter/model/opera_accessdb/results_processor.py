    ...
    def update_production_capacities(self):
        ...
        for index, row in self.df.iterrows():  # iterate through input list
            ...
            if not found.empty:
                ...
                log.debug(f"Found updated capacity for {asset_name}: {updated_capacity_in_GW} GW in range [{min_capacity_range:.2f}-{max_capacity_range:.2f}]")
                id = row['id']
                ...
                if asset and hasattr(asset, 'power'):
                    ...
                    #asset:esdl.GenericProducer = asset
